...
public class AutorService {
	...
	public ListaResposta addAutor(Autor autor, String editora_id) {
		...
		if (dataMinima.compareTo(autor.getData_nascimento().toLocalDate()) > 0
				|| dataMaxima.compareTo(autor.getData_nascimento().toLocalDate()) <= 0) {
	...
	/*
	 * public String deleteAutor (String id) { try { Long id_long = parseLong (id);
	 * 
	 * if (id == null || id_long == NaN || autorRepository.findById
	 * (id_long).isEmpty () ) { return
	 * "ID de andar inexistente ou fora de formato."; }
	 * 
	 * Autor autor = autorRepository.findById (id_long).get ();
	 * autorRepository.delete (autor); return "";
	 * 
	 * } catch (Exception e) { return "o ID tem de ser um n.º longo."; } }
	 */
